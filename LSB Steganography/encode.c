...
Status do_encoding(EncodeInfo *encInfo)
{
    ...
    {
	printf("INFO: Opening required filesn");
	...
	if(check_capacity(encInfo) == e_success)
	{
	    ...
	    { 
		printf("INFO: Copied bmp header successful\n");

		if(encode_magic_string(MAGIC_STRING,encInfo) == e_success)
		{
		    printf("INFO: Encoded magic string Signature is successful\n");
		    strcpy(encInfo -> extn_secret_file, strstr(encInfo -> secret_fname, "."));

		    if(encode_extn_size(strlen(encInfo -> extn_secret_file),encInfo) == e_success)
		    {	
			printf("INFO: Encoded secret file extension size is succesful\n");	
			...
			if(encode_secret_file_extn(encInfo -> extn_secret_file,encInfo) == e_success) 
			{
			    ...
			    {
				printf("INFO: Encoded secret file size is successful\n");

				if(encode_secret_file_data(encInfo) == e_success)
				{
				    printf("INFO: Encoded secret file data is successful\n");	

				    if(copy_remaining_img_data(encInfo -> fptr_src_image,
						encInfo -> fptr_stego_image) == e_success)
				    {
					printf("INFO: Copied remaining image data successful\n");
				    }	
				    else
				    {
		    ...
		    {
			printf("INFO: Failed to encode secret file size extension\n");
			return e_failure;
		    }
		}
		else
	    ...
	    { 
		printf("INFO: Failed to copy header\n");
		return e_failure;
	    }
	}
	else
